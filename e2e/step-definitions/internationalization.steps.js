...
defineFeature((feature), (test) => {
    ...
    test("We are login into Viade and we check all views are internationalized",({given,when,then}) => { 
        ...
        when("The user press the boton Language or Idioma (depending on the language you have selected) and select the other language", async () => {
            ...
            await expect(page).toMatchElement("div > div > section > nav > div > div > div > button", { id: "btnLanguage" ,waitUntil: "load", timeout: 0});
            ...
            await expect(page).toMatchElement("div > div > section > nav > div > div > div > div > a > div", { id: "btnEng" ,waitUntil: "load", timeout: 0});
