...
const feature = loadFeature('./test/features/loadCombo.feature');
...
defineFeature(feature, test => {
  ...
  test('The are some routes on the POD', ({ given, when, then ,and}) => {
    ...
    when('I press the button "Cargar rutas del POD"', async () => {
      // await expect(page).toFillForm('form[name="register"]', {
        ...
        // remail: email,
      // })
      await expect(page).toClick('button', { text: Cargar rutas del POD' })
    });
